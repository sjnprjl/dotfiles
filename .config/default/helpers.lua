...
local helpers = {}
helpers.split  = function(s, delimiter)
    result = {};
...
function helpers.focus_on_last_in_history( screen )
...
function helpers.alsa_volume(step) 
    local cmd 
    local sign
    if step == 0 then 
        ...
        cmd = "amixer sset Master "..tostring(math.abs(step)).."%"..sign
    end
